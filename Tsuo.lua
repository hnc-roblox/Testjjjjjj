...
local Window = Fluent:CreateWindow({Title = "HNC Hub",SubTitle = "By HNC Roblox",TabWidth = 120,Size = UDim2.fromOffset(455, 320),Acrylic = false,Theme = "Aqua",MinimizeKey = Enum.KeyCode.End})
local Tabs = {
  Home = Window:AddTab({Title = "Cứt", Icon = ""}),
  ...
  Mirage = Window:AddTab({Title = "RaceV4", Icon = ""}),
...
if IsPc then
  if not string.find(executor, "Wave") or not string.find(executor, "AWP") then
    Fluent:Notify({Title = "HNC Hub",Content = "Finished Loading",SubContent = "By HNC Roblox",Duration = 15})
  else
    Fluent:Notify({Title = "HNC Hub",Content = "By HNC Roblox",SubContent = "Anti Ban Enabled",Duration = 15})
  end
else
  Fluent:Notify({Title = "HNC Hub",Content = "Attack/M1",SubContent = "Fast Attack Enabled",Duration = 15})
end

-- ==========================
-- Auto Farm Chest + Auto Change Team
-- ==========================
local LocalPlayer = game:GetService("Players").LocalPlayer
local ReplicatedStorage = game:GetService("ReplicatedStorage")

local FarmingChest = false
local SelectedTeam = "Marines"
...
local function setClearMap(toggle)
    if toggle then
        HiddenObjects = {}
        for _, obj in pairs(workspace:GetDescendants()) do
            if obj:IsA("BasePart") or obj:IsA("Decal") then
                if not isMob(obj.Parent) then
                    table.insert(HiddenObjects, {Part = obj, Transparency = obj.Transparency})
                    obj.Transparency = 1
                end
            end
        end
    else
        for _, info in pairs(HiddenObjects) do
            if info.Part and info.Part.Parent then
                info.Part.Transparency = info.Transparency
            end
        end
        HiddenObjects = {}
    end
...
-- UI Elements
-- ==========================
Tabs.Home:AddToggle("FarmChest", {
    Title = "(BETA) Auto Farm Chest [TP]",
    Default = false,
...
Tabs.Home:AddDropdown("TeamSelect", {
...
Tabs.Home:AddToggle("ClearMap", {
    Title = "Clear Map (Lag Fix)",
    Default = false,
    Callback = function(Value)
        ClearMap = Value
        setClearMap(Value)
...
local Initialize = Tabs.Settings:AddToggle("Initialize", {Title = "Fast Attack", Description = "", Default = true})
...
local Byp = Tabs.Settings:AddToggle("Byp", {Title = "Bypass Teleport", Description = "", Default = false})
Byp:OnChanged(function(Value)
  _G.Bypass = Value
end)
local SafeModes = Tabs.Settings:AddToggle("SafeModes", {Title = "Safe Mode", Description = "safe ur health if low", Default = false})
